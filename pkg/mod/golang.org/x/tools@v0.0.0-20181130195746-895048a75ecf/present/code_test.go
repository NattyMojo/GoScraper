...
`)
	...
	tests := []struct {
		...
		},
		{
			...
			sourceFile: "scrapi.go",
			cmd:        ".code scrapi.go",
			err:        "scrapi.go:0: nope",
		},
		{
			name:       "from func main to the end",
			readFile:   read(helloTest, nil),
			sourceFile: "scrapi.go",
			cmd:        ".code scrapi.go /func main/,",
			Code: Code{
				Ext:      ".go",
				FileName: "scrapi.go",
				Play:     false,
		...
		},
		{
			...
			readFile:   read(helloTest, nil),
			sourceFile: "scrapi.go",
			cmd:        ".code scrapi.go /function main/",
			err:        "scrapi.go:0: no match for function main",
		},
		{
			name:       "all code with  numbers",
			readFile:   read(helloTest, nil),
			sourceFile: "scrapi.go",
			cmd:        ".code -numbers scrapi.go",
			Code: Code{
				Ext:      ".go",
				FileName: "scrapi.go",
				Raw:      helloTest,
